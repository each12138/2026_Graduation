...
class TfPoseProvider:
    def __init__(self):
        ...
        try:
            ...
            self.node = _create_tf_pose_node()
            self.executor = SingleThreadedExecutor()
            self.executor.add_node(self.node)
        except Exception as exc:
            self.close()
            raise RuntimeError(self._build_init_error(exc)) from exc

        self._running = True
        self._spin_thread = threading.Thread(target=self._spin_loop, daemon=True)
        self._spin_thread.start()

    def _build_init_error(self, exc):
        rmw_impl = os.getenv("RMW_IMPLEMENTATION", "<unset>")
        ros_domain_id = os.getenv("ROS_DOMAIN_ID", "<unset>")
        cyclone_uri = os.getenv("CYCLONEDDS_URI", "<unset>")
        detail = str(exc).strip() or exc.__class__.__name__
        return (
            "ros2_domain_create_failed: unable to create the TF listener ROS2 node. "
            f"RMW_IMPLEMENTATION={rmw_impl}, ROS_DOMAIN_ID={ros_domain_id}, "
            f"CYCLONEDDS_URI={cyclone_uri}. Original error: {detail}. "
            "Check that the ROS2 environment is sourced, the DDS middleware is installed "
            "and configured correctly, and the selected ROS domain/network interface is valid."
        )
